...
function openMoyasarPayment(orderData){
  ...
  wrap.style.cssText='position:fixed;inset:0;background:rgba(0,0,0,.5);z-index:9999;display:flex;align-items:center;justify-content:center;backdrop-filter:blur(8px)';
  const box=document.createElement('div');
  box.style.cssText='background:#FDFAF5;border:1px solid rgba(196,154,42,.35);border-radius:20px;padding:32px;width:90%;max-width:480px;max-height:90vh;overflow-y:auto;box-shadow:0 24px 80px rgba(0,0,0,.6);position:relative';
  ...
  title.style.cssText='font-family:Tajawal,sans-serif;font-size:20px;font-weight:800;margin-bottom:8px;text-align:center;color:#3D2B1F';
  title.textContent='💳 إتمام الدفع';
  const sub=document.createElement('p');
  sub.style.cssText='text-align:center;color:#8A7060;font-size:14px;margin-bottom:24px';
...
function showThankYouPage(orderData){
  ...
  const items=Object.entries(src).map(([id,item])=>`<div style="display:flex;justify-content:space-between;margin-bottom:8px;color:#4A3728"><span>${item.icon} ${item.name}</span><span style="color:var(--gold)">${item.price.toLocaleString('ar-SA')} ر.س</span></div>`).join('');
